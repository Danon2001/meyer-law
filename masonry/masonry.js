const MasornyActiveClassName = 'masornyActive';
...
class Masonry {
    ...
    setParameters(){
        ...
        const widthImage = (containerWidth - this.settings.gap * 
            (this.settings.columns-1)) /this.settings.columns;

        this.childrenData = this.childrenData.map((child) => ({
            ...child,
            currentWidth: widthImage,
            currentHeight: Math.floor(widthImage * child.originHeight
                /child.originWidth)
        }));
        ...
        this.childrenData.forEach((child,i) => {
            heightColumns[i % this.settings.columns] += child.currentHeight +
            this.settings.gap;
        ...
        (heightColumn - minHeightColumn) / sizeColumns[i]);
        ...
        this.containerNode.classList.add(MasornyActiveClassName);
